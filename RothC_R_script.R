...
opt_tstep <- df_opts[[1,'opt_tstep']]
opt_init <- df_opts[[1,'opt_init']]
df_pools <- read.csv('RothC_input.dat', skip = 6, header = 1, nrows = 1, sep = '')
df_head <- read.csv('RothC_input.dat', skip = 9, header = 1, nrows = 1, sep = '')# sep = '' can be removed if file is comma delimited
clay <- df_head[[1,'clay']]
...
df <- read.csv('RothC_input.dat', skip = 12, header = 1, sep = '')# sep = '' can be removed if file is comma delimited
...
# set initial pool values (supply zeros to active pools (DPM, RPM, Bio, and Hum) for spin-up method)
IOM <- df_pools[[1,'IOM']]
DPM <- df_pools[[1,'DPM']]
RPM <- df_pools[[1,'RPM']]
Bio <- df_pools[[1,'Bio']]
...
# set inital radiocarbon (50000 for IOM and zeros for active pools for spin-up method)
IOM_Rage <- df_pools[[1,'IOM_age']]
DPM_Rage <- df_pools[[1,'DPM_age']]
RPM_Rage <- df_pools[[1,'RPM_age']]
Bio_Rage <- df_pools[[1,'Bio_age']]
Hum_Rage <- df_pools[[1,'Hum_age']]


# setting up timeStep variable to reduce the number of places the conditional input needs to be edited
if(opt_tstep %in% c(1,"m","month","monthly","M","Month","Monthly")){
  timeStep <- "monthly"
} else if(opt_tstep %in% c(2,"d","day","daily","D","Day","Daily")){
  timeStep <- "daily_with_leapyears"
} else if(opt_tstep %in% c(3)){
  timeStep <- "daily_without_leap"
}

# no leap years accounted for in spin-up initialisation
if(timeStep == "monthly"){
  timeFact <- 12
} else if(timeStep %in% c("daily_with_leap","daily_without_leap")){
  timeFact <- 365
}

# for defining where the scenario starts (timeFact+1 or 0+1)
if(opt_init %in% c(1,3)){
  init_rows <- timeFact
} else if(opt_init %in% c(2)){
  init_rows <- 0
}
...
# zero is used when calculating the radiocarbon age
zero <- 0
...
month_list <- list()
month_list[[1]] <- data.frame(1,j, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, DPM, RPM, Bio, Hum, IOM, SOC, co2_tot)
colnames(month_list[[1]]) <- c('Year','Month','Pl_inp_t_C_ha','OA_inp_t_C_ha','TEMP_C','RM_Temp','RAIN_mm','PEVAP_mm','SMD_mm','RM_Moist','PC','RM_PC','DPM_t_C_ha','RPM_t_C_ha','Bio_t_C_ha','Hum_t_C_ha','IOM_t_C_ha','SOC_t_C_ha',"CO2_t_C_ha")

# run RothC after initialisation
for(i in seq(init_rows+1, nsteps,1)){
  ...
  if(timeStep == "daily_with_leap"){ 
    if(is_leap_year(t_year[i])){
      timeFact <- 366
    } else {
      timeFact <- 365
    }
  ...
  month_list[[i+1-timeFact]] <- data.frame(df[[i, 't_year']], df[[i,'t_month']],Pl_inp, OA_inp, TEMP, RM_Temp, RAIN, PEVAP, SMD, RM_Moist, PC, RM_PC, DPM, RPM, Bio, Hum, IOM, SOC, co2_tot)
  colnames(month_list[[i+1-timeFact]]) = c('Year','Month','Pl_inp_t_C_ha','OA_inp_t_C_ha','TEMP_C','RM_Temp','RAIN_mm','PEVAP_mm','SMD_mm','RM_Moist','PC','RM_PC','DPM_t_C_ha','RPM_t_C_ha','Bio_t_C_ha','Hum_t_C_ha','IOM_t_C_ha','SOC_t_C_ha',"CO2_t_C_ha")
